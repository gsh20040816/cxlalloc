// Taken from https://github.com/kaist-cp/memento/blob/main/evaluation/performance/hash/hash/Clevel-rust/clevel_rust/src/lib.rs
...
pub extern "C" fn pool_create(
    ...
    unsafe {
        SEND = Some(core::array::from_fn(|_| send.clone()));
        RECV = Some(recv);
        HANDLES = Some(core::array::from_fn(|_| None));
    }
