...
pub trait Erase<'raw, 'root>: Sized {
    fn erase(self, heap: &Heap<'raw>) -> Site;
}

impl<'raw, 'root, T> Erase<'raw, 'root> for &'root mut Option<crate::Box<T>> {
    fn erase(self, heap: &Heap<'raw>) -> Site {
        Site::Data(heap.pointer_to_offset(NonNull::from(self)))
    }
}

impl<'raw, 'root, T> Erase<'raw, 'root> for &'root mut crate::Root<'raw, T> {
